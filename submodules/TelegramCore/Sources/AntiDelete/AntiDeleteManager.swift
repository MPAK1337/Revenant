...
public final class AntiDeleteManager {
    ...
    private func saveArchive() {
        do {
            // Recovered message content is encrypted at rest, not left in a plaintext plist.
            try SecureStore.shared.setSecure(archivedMessages, forKey: archiveKey)
        } catch {
    ...
    private func loadArchive() {
        // One-time move of any legacy plaintext archive into the encrypted store.
        SecureStore.shared.migratePlaintextIfNeeded(userDefaultsKey: archiveKey, storeKey: archiveKey)
        do {
            archivedMessages = try SecureStore.shared.getSecure([ArchivedMessage].self, forKey: archiveKey) ?? []
        } catch {
